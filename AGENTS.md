...
Open: v2 multi-brand retrain is PREPPED and awaiting hand launch (`finetune/fireworks_launch_v2.sh` - firectl refuses agent-run mutations). Training set built by `finetune/build_train_v2.py`: 13,491 rows (jira 5,081 + AmEx 5,090 + Pixel 3,620) with one-line code defs in every prompt (`jira_code_defs.json`, `pixel_code_defs.json` authored for training; the validated AmEx sheet reused); hyperparameters identical to v1 so the delta is data-attributable. Held out (split_v2.json): Netflix entirely (the unseen-transfer measurement), jira ratified 339 + v1 val 300, AmEx calib 300, Pixel holdout 300 (`pixel/holdout_300_ids.txt`), sft val 300 (100/brand). At eval time score Netflix both bare-list and with a defs sheet authored-then-paired-tested per the onboarding procedure (the old Netflix defs stay deleted). Production coder decision, prod config flip, and coding the 26,070 held answers all await Tyler's go. Remaining fleet to collect (trimmed panel): AG1, athenahealth, Purple, PwC, Nest - 2,440 prompts, ~26,840 answers at the full engine set; per-category calibration labeling is ~$10-25 on the Batch API if the subagent rule is ever lifted for prod.
