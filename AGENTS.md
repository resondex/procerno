...
- jira, no regression: held-out 639 outcome 93.9 / framing 92.3 / top_pick 95.8 / F1 80.8 (v1: 93.1/90.5/95.3/79.9); ratified 339 vs HUMAN 92.0/89.1/95.0.
...
Open: production coder decision (v2, bare-list prompting) awaits Tyler's go, as do prod config flip and coding the 26,070 held answers. Remaining fleet to collect (trimmed panel): AG1, athenahealth, Purple, PwC, Nest - 2,440 prompts, ~26,840 answers at the full engine set; per-category calibration labeling is ~$10-25 on the Batch API if the subagent rule is ever lifted for prod.
